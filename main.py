...
#We will use different classification models and imbalanced class rectifiers to train our model. We will be calculating the Accuracy, Precision, Recall, F1-Score and the Matthews correlation coefficient for model evaluation. Since our data is highly imbalanced, Accuracy can be misleading and therefore we will be computing other mentioned performance metrics.
#Our aim is to build a model that has a high recall as that would mean it is sensitive to fraud detection. This does not mean that it should have a very low precision as that would result in a lot of spams due to a lot of valid transactions being misclassified as fraud. So, the model should be somewhat balanced and for this we look at F1-score and Matthews correlation coefficient.
...
#We can run different algorithms along with rectifiers, vary the number of features used and then select the best combination based on performance metrics and computational cost (given by execution time).
#Feel free to build your own web app using streamlit. For more information, check out the documentation of streamlit (https://docs.streamlit.io/en/stable/#). If you have any difficulty or any doubts regarding this topic kindly comment below. Your support is always highly appreciated and will motivate me to write more articles.
